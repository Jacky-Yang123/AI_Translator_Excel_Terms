# pages/prompt_generator.py - 提示词生成器
    ...
    with col2:
        ...
        if uploaded_role is not None:
            try:
                ...
                personality_col_select = st.selectbox(
                    "💬 选择性格描述列",
                    options=cols,
                    index=min(1, len(cols)-1) if len(cols) > 1 else 0,
                ...
                if st.button("📥 加载角色性格库", key="prompt_load_role_personality"):
                    if translator.load_role_personality(df, role_col_select, personality_col_select):
...
"""
        ...
        for i in range(num_batches):
            ...
            term_base_prompt = ""
            if term_base_loaded:
                term_base_prompt = translator.build_term_base_prompt(all_text_in_batch, target_language)
            else:
                term_base_prompt = "\n\n### 术语库匹配：\n无术语库加载，跳过术语匹配。"
